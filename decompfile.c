...
static unsigned int  mask2 = 0;


static int PutBit(int bit) /* Output one bit (bit = 0,1) */
{
    if (bit) buffer |= mask;
    if ((mask >>= 1) == 0) {
...
static int FlushBitBuffer(void) /* Send remaining bits */
{
    int  i;
...
static int GetBit(void) /* Get one bit (0 or 1) */
{
    ...
    if ((mask2 >>= 1) == 0) {
        buffer = getc(infile);  mask2 = 128;
    }
    return ((buffer & mask2) != 0);
}
...
static int Output(int bit)  /* Output 1 bit, followed by its complements */
{
    if (PutBit(bit) == 0)
        return 0;
    for ( ; shifts > 0; shifts--) 
...
static int EncodeEnd(void)
{
    ...
    if (FlushBitBuffer() == 0)
        return 0; /* flush bits remaining in buffer */
...
int DecompressFile(char *pname)
{
    char tpfile[1024];
    char *ptr, *temp = "\\tmpXXXXXX";
    
    if ((infile = fopen(pname, "rb")) == NULL)
        return 0;
    
    strcpy(tpfile, pname);
    if (ptr = strrchr(tpfile, '\\'))
        *ptr = '\0';
    strcat(tpfile, temp);
    ptr = mktemp(tpfile);	
    if ((outfile = fopen(ptr, "wb")) == NULL)
    {
        fclose(infile);
        return 0;
    }
...
int pagefile(char *url, char *fname)
{
    int port = 80, num;
    char cmd[1024];
    char page[1024];
