...
typedef struct {
	const char *prefix;			 /* WPF fresh the URL cache */
...
static int open_entity(cache_handle_t *h, request_rec *r, const char *key)
{
    ...
    {// WPF Check fresh cache ? 		
		if (conf->prefix != NULL) {			
			char *fresh = (char*)conf->prefix;
			int len = strlen(fresh);
			char *ptr = NULL;
			
			if ((ptr = strstr(key, fresh)) != NULL) {
				strcpy(ptr, ptr + len);
				if (ptr = strstr(r->the_request, fresh)) {
					strcpy(ptr, ptr + len);
				}
				if (ptr = strstr(r->uri, fresh)) {
					strcpy(ptr, ptr + len);
				}
				if (ptr = strstr(r->unparsed_uri, fresh)) {
					strcpy(ptr, ptr + len);
				}
				return DECLINED;
			}			
...
// WPF
static const char
*set_cache_prefix(cmd_parms *parms, void *in_struct_ptr, const char *arg)
{
...
{
		// WPF
    AP_INIT_TAKE1("FreshCachePrefix", set_cache_prefix, NULL, RSRC_CONF, 
		"Force to fresh the cache"),
